...
// 选择 IO 轮询行为：
// ZMOTION_DRIVER_USE_MULTAXIS_IO: IO 0-5 控制多轴组合，IO 8-11 控制 mimic group
// ZMOTION_DRIVER_USE_PERAXIS_IO:  IO 0-7 pair 控制单轴，IO 8-11 控制 mimic
// group
#define ZMOTION_DRIVER_USE_MULTAXIS_IO
...
class ZMotionDriverNode::Impl
{
  ...
  void PublishFeedback()
  {
    ...
    if (!this->sdk->IsConnected())
    {
      ...
      {
        this->TriggerEmergencyStopLocked("controller disconnected");
      }
      return;
    }
  ...
  void PollIoInputs()
  {
    // 统一的 IO 轮询入口，根据宏定义选择 dispatch 策略
    std::lock_guard<std::mutex> lock(this->mutex);
    if (!this->configured)
    {
      return;
    ...
    if (!this->sdk->IsConnected())
    {
      if (!this->emergency_stop)
      {
        this->WriteLogLocked("controller_status",
                             "controller disconnected, triggering e-stop");
        this->TriggerEmergencyStopLocked("controller disconnected");
      }
      return;
    }
...
#if defined(ZMOTION_DRIVER_USE_MULTAXIS_IO)
      this->DispatchIoTriggerMultiAxisLocked(i, value);
#elif defined(ZMOTION_DRIVER_USE_PERAXIS_IO)
      this->DispatchIoTriggerPerAxisLocked(i, value);
#else
      // 默认使用单轴模式
      this->DispatchIoTriggerPerAxisLocked(i, value);
#endif
  ...
  void DispatchIoTriggerMultiAxisLocked(std::size_t index, int current_value)
  {
  ...
  void DispatchIoTriggerPerAxisLocked(std::size_t index, int current_value)
  {
    if (index >= this->io_inputs.size())
    {
      return;
    ...
    {
      // IO 外范围或未配置的 IO：仅发布状态
      this->WriteLogLocked("io_action",
                           "io_" + std::to_string(cfg.io_id) +
                               " triggered but no control action configured");
    }
  }

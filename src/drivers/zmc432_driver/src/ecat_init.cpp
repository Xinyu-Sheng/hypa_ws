                                     ...
                                     EcatInitInfoSet EcatInfo, int ApiOutTime)
{
  ...
  if (EcatInfo.InitStructFlag == 1)
  {
    ...
    EcatInfo.DriveIoSpa = 16;  // 一个驱动器映射多少个IO到控制器上，需要是8的倍数
    memset(
    ...
    memset(EcatInfo.DcOffsetFlag, 0,
           sizeof(EcatInfo.DcOffsetFlag));  // 第N个节点类型的驱动器是否需要打开DC偏移功能，0：关闭DC偏移，1：打开DC偏移
                                            // ,数组下标即是节点ID
    memset(
  ...
  for (int i = 0; i < EcatInfo.LocalAxisNum; ++i)
  {
    Iresult += ZAux_Direct_SetAxisAddress(handle, EcatInfo.LocalAxisId + i,
                                          (-1 << 16) + i);
    Iresult += ZAux_Direct_SetAtype(handle, EcatInfo.LocalAxisId + i, 1); (-1 << 16) + i);//将本地轴0-->i映射到轴LocalAxisId-->LocalAxisId+i
    // 3、DC同步时钟的设置
    ...
    // 4、扫描总线驱动器，进行DC偏移时间的设置
    int ScanOkFlag = 0;
    // 2. SLOT_SCAN: 物理扫描：确认网线连接正常，识别所有从站。
    for (int i = 0; i < 3; ++i)
    {
      ...
      if (1 == ScanOkFlag)  // 如果有扫描到驱动器
      {
        ...
        for (int i = 0; i < ScanNodeNum; i++)
        {
          ...
          if (EcatInfo.DcOffsetFlag[i] == 1)
          {
            ...
            // DC偏移设置失败
            if (((ServoPeriod * 1000 * EcatInfo.DcOffsetTime[i] - ZmlInfo) >
                 5) ||
                (ZmlInfo != NodeInfo))
            {
          ...
          for (int j = 0; j < NodeAxisNum; j++)
          {
            ...
            if ((Drive_Device == ElmoDevice) && (Drive_Vender == ElmoVender))
            {
              ...
              // ELMO的驱动器需要关闭总线时钟优化
              Iresult += ZAux_Execute(handle,
                                      "SYSTEM_ZSET = CLEAR_BIT(7, SYSTEM_ZSET)",
                                      ReceBuff, 256);
              ...
              // ELMO的驱动器需要自定义PDO
              Iresult +=
                  ZAux_BusCmd_SDOWrite(handle, SlotId, i, 0x1c12, 0, 5,
                                       0);  // 禁用RxPDO,禁用后才可以修改内容
              Iresult +=
                  ZAux_BusCmd_SDOWrite(handle, SlotId, i, 0x1c1c, 0, 5,
            ...
            else
            {
              ...
              {
                // 6. 设置 DRIVE_IO: I/O 映射：如果驱动器带 IO，分配其起始地址。
                int StartIdTemp =
                    EcatInfo.DriveIoStara + EcatInfo.DriveIoSpa * (BusAxisNum);
                sprintf(cmdbuff, " DRIVE_IO(%d + %d) = %d",
                ...
                Iresult += ZAux_Direct_SetFwdIn(
                    handle, EcatInfo.DriveAxisStart + BusAxisNum,
                    StartIdTemp + 1);
          ...
          ***************************************************************************************/
          ...
          if ((Drive_Device == 0x1AB0) && (Drive_Vender == 0x41B))
          {
            for (int k = 0; k < NodeAxisNum; k++)
            {
              ...
              ZAux_BusCmd_SDOWrite(handle, SlotId, i, 0x6011 + k * 0x800, 0, 5,
                                   1);
              // 设置扩展脉冲轴INVERT_STEP脉冲输出模式  INVERT_STEP=0
              ZAux_BusCmd_SDOWrite(handle, SlotId, i, 0x6012 + k * 0x800, 0, 6,
                                   0);
            }
          }
        }
        ...
        // 7. SLOT_START: 启动总线：切换到 OP 状态，开始周期性通信。
        ...
        {
          // 读取在线命令的应答， 对没有接收应答的命令有用
          ZMC_ExecuteGetReceive(handle, ReceBuff, 1000, &puiread,
                                &pbifExcuteDown);
          if (ReceBuff[0] != 0)
          {
            ReceBuff[2] = 0;
        ...
        if (0 == strcmp("-1", ReceBuff))
        {
          MyDelayMs(
              3000,
              &OutTime);  // 延迟3秒，等待驱动器时钟同步，不同驱动器时间不同，具体根据驱动器调整延时
          // 9. DRIVE_CONTROLWORD:
          // 清驱动器故障：发送控制字复位驱动器的历史报警。
          //"开始清除驱动器报警"
